...
export default function Navbar() {
  ...
  return (
    <>
      <div className="fixed top-6 inset-x-0 px-6 z-[100] font-inter">
        ...
        >
          ...
          <a href="#" className="flex items-center gap-2 text-2xl font-extrabold text-gray-900 dark:text-white tracking-tighter">
            <div className="w-8 h-8 rounded-full bg-[#00BFA6] flex items-center justify-center font-black italic text-sm text-black">
              G
            </div>
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.label}
                href={link.href}
                className="relative text-sm font-bold text-gray-700 dark:text-white/90 hover:text-[#00BFA6] transition-colors duration-200"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button
              onClick={toggleTheme}
              className="flex text-gray-700 dark:text-white/70 hover:text-[#00BFA6] text-xl transition-colors w-10 h-10 items-center justify-center hover:bg-black/5 dark:hover:bg-white/5 rounded-full"
            ...
            <button
              aria-label="Search"
              className="hidden md:flex text-gray-700 dark:text-white/70 hover:text-[#00BFA6] text-xl transition-colors w-10 h-10 items-center justify-center hover:bg-black/5 dark:hover:bg-white/5 rounded-full"
            >
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="fixed inset-0 z-[90] bg-white/98 dark:bg-[#0A0A0A]/98 backdrop-blur-xl flex flex-col items-center justify-center gap-8">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              onClick={() => setMobileOpen(false)}
              className="text-2xl font-extrabold text-gray-900 dark:text-white hover:text-[#00BFA6] transition-colors"
            >
              {link.label}
            </a>
          ))}
          <button className="mt-4 text-sm font-bold px-8 py-3 bg-[#131313] dark:bg-white text-white dark:text-black rounded-full">
            Start a Project
          </button>
        </div>
      )}
